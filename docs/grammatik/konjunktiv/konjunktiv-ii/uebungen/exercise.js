...
const exerciseLabels = {
  "80": "80. Bilden Sie Konditionalsätze mit dem Konjunktiv II der Gegenwart!",
  "81": "81. Bilden Sie Konditionalsätze mit dem Konjunktiv II der Vergangenheit!",
  "82": "82. Bilden Sie Konditionalsätze mit Passiv-Formen im Konjunktiv II!",
  "83": "83. Bilden Sie Konditionalsätze mit dem Konjunktiv II!",
  "84": "84. Formen Sie die Konditional-Angaben in Konditionalsätze um!",
  "85": "85. Benutzen Sie den Konjunktiv II!"
};
...
function extractExercises(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Skip subsection headers like **(1) Wünsche:**
    ...
    if (itemMatch) {
      ...
      if (arrowIndex !== -1) {
        ...
        // Handle multiple answers separated by / (for exercise 85)
...
function updateExerciseSelect() {
  ...
  // Show exercises 80, 81, 82, 83, 84, 85
  const allowedExercises = ["80", "81", "82", "83", "84", "85"];
...
function checkExercise() {
  state.currentItems.forEach(itemDiv => {
    ...
    // Handle multiple correct answers separated by / (for exercise 85)
    const correctAnswers = correctAnswer.split("/").map(a => normalizeText(a.trim()));
    const normalizedCorrect = correctAnswers[0]; // Use first answer for comparison
