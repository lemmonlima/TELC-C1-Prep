const state = {
  ...
  pools: {
    ...
    adjectives: {},
    dativPronouns: ["mir", "dir", "ihm", "ihr", "uns", "euch", "ihnen", "dem", "der", "den"],
    akkusativPronouns: ["mich", "dich", "ihn", "sie", "es", "uns", "euch", "sie", "den", "die", "das"]
  }
};
...
function buildSentence(sentence) {
  ...
  // Buscar todos los tokens: {prep-erg:...}, {dat:...}, {akk:...}
  const tokenRegex = /\{(prep-erg|dat|akk):([^}]+)\}/g;
  let match;
  const matches = [];
  ...
  for (let i = matches.length - 1; i >= 0; i--) {
    ...
    } else if (match.type === "dat" || match.type === "akk") {
      // Tokens dat/akk: pueden ser una palabra (pronombres) o múltiples palabras (medidas)
      const content = match.content.trim();
      const words = content.split(/\s+/);
      
      if (words.length === 1) {
        // Una sola palabra: pronombre personal
        const word = words[0];
        blankCount += 1;
        ...
        if (match.type === "dat") {
          pool = state.pools.dativPronouns;
          fallback = state.pools.dativPronouns;
        } else if (match.type === "akk") {
          pool = state.pools.akkusativPronouns;
          fallback = state.pools.akkusativPronouns;
        }
        
        replacementHtml = renderSelect(word, pool, fallback);
      } else {
        // Múltiples palabras: analizar palabra por palabra (medidas como "sieben Stockwerke")
        const analyzed = analyzePrepErgContent(content);
...
function extractExercises(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Detect exercise header (## 11. ... or ## 12. ...)
...
function updateExerciseSelect() {
  ...
  // Show exercises 11, 12
  const allowedExercises = ["11", "12"];
