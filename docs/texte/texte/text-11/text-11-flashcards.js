...
function highlightWordInSentence(sentence, data) {
  ...
  // Primero, si hay una frase completa (con espacios), resaltarla
  ...
  // Luego, resaltar partes individuales que no estén ya dentro de un span
...
function updateCard() {
  ...
  // Parte trasera: traducción + ejemplo
...
async function loadDeck() {
  try {
    const response = await fetch("text-11-explanations.json", { cache: "no-store" });
    if (!response.ok) {
    ...
    // Crear deck: todas las palabras que tengan oración y traducción
    const deck = entries
      .map(([id, value]) => ({
        id,
        word: value.word,
