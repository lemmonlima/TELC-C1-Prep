...
async function loadExplanations() {
  try {
    const response = await fetch('kuenstliche-intelligenz-im-studium-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'kuenstliche-intelligenz-im-studium-flashcards.html';
    });
  }
