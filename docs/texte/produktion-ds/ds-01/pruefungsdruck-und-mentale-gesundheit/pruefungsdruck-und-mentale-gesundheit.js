...
async function loadExplanations() {
  try {
    const response = await fetch('pruefungsdruck-und-mentale-gesundheit-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'pruefungsdruck-und-mentale-gesundheit-flashcards.html';
    });
  }
