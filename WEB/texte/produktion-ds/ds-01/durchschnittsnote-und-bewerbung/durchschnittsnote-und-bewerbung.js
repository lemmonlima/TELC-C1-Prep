...
async function loadExplanations() {
  try {
    const response = await fetch('durchschnittsnote-und-bewerbung-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'durchschnittsnote-und-bewerbung-flashcards.html';
    });
  }
