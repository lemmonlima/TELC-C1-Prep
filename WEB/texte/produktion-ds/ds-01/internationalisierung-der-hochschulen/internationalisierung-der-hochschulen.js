...
async function loadExplanations() {
  try {
    const response = await fetch('internationalisierung-der-hochschulen-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'internationalisierung-der-hochschulen-flashcards.html';
    });
  }
