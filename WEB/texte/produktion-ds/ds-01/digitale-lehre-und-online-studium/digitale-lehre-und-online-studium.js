...
async function loadExplanations() {
  try {
    const response = await fetch('digitale-lehre-und-online-studium-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'digitale-lehre-und-online-studium-flashcards.html';
    });
  }
