...
async function loadExplanations() {
  try {
    const response = await fetch('anonymitaet-in-sozialen-netzwerken-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'anonymitaet-in-sozialen-netzwerken-flashcards.html';
    });
  }
