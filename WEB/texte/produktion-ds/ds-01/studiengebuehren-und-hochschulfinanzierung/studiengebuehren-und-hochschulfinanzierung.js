...
function showExplanation(id, markedText, fromTextClick, parentId) {
  ...
  // Solo guardamos la posición de scroll cuando venimos de un click en el texto principal.
  if (fromTextClick) {
...
async function loadExplanations() {
  try {
    const response = await fetch('studiengebuehren-und-hochschulfinanzierung-explanations.json', { cache: 'no-store' });
    if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (textFlashBtn) {
    textFlashBtn.addEventListener('click', () => {
      window.location.href = 'studiengebuehren-und-hochschulfinanzierung-flashcards.html';
    });
  }
