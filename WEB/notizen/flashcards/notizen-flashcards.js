...
function buildDeck(listData, explanationData) {
  const deck = [];
  const items = listData.items || [];
  for (const item of items) {
    const explanationId = item.explanationId;
    const text = (item.text || "").trim();
    if (!explanationId || !text) continue;
    const entry = explanationData[explanationId];
...
async function loadDeck() {
  try {
    const [listRes, explRes] = await Promise.all([
      fetch("../notizen.json", { cache: "no-store" }),
      fetch("../notizen-explanations.json", { cache: "no-store" })
    ]);
    if (!listRes.ok || !explRes.ok) throw new Error("Failed to load");
    const listData = await listRes.json();
    const explanationData = await explRes.json();
    const deck = buildDeck(listData, explanationData);
